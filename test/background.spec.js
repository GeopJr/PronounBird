...
describe("Background script", () => {
  ...
  it("Finds bearer and csrf from request headers", () => {
    ...
    const csrf = Tokens.tempCsrfToken;
  ...
  it("Ignores bad cookie", () => {
    ...
    let csrf = Tokens.csrfToken;
    expect(csrf, "no csrf after bad cookie").to.equal(undefined);
  });

  it("Finds csrf from cookie", () => {
    // good cookie
    Tokens.getTheCookieTokens({
      cookie: {
        name: "ct0",
    ...
    expect(csrf, "csrf found after good cookie").to.equal("csrf");
  });
